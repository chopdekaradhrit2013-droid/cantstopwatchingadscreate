    ...
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        ...
        <form className="mt-6 space-y-3 rounded-2xl border border-neutral-200 bg-white p-5" onSubmit={(e) => e.preventDefault()}>
          ...
          <label className="block text-sm">Logo URL<input value={brand.logo} onChange={(e) => updateBrand({ logo: e.target.value })} className="mt-1 w-full rounded-xl border px-3 py-2" /></label>
          ...
          <label className="block text-sm">X / Twitter<input value={brand.twitter} onChange={(e) => updateBrand({ twitter: e.target.value })} className="mt-1 w-full rounded-xl border px-3 py-2" /></label>
      ...
      <aside>
        ...
        <div className="mt-3 rounded-2xl border border-neutral-200 bg-white p-5">
          ...
          <p className="mt-2 text-xs text-neutral-400">{brand.industry} · {brand.followers.toLocaleString()} followers</p>
        ...
        {publicView && (
          <div className="fixed inset-0 z-50 overflow-auto bg-[#f6f5f2] p-6">
            ...
            <div className="mx-auto max-w-xl rounded-3xl border bg-white p-8">
              ...
              <a href={brand.website} className="mt-3 inline-block text-sm underline" target="_blank" rel="noreferrer">{brand.website}</a>
            </div>
          </div>
        )}
      </aside>
